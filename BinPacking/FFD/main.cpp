...
int firstFitDecreasing(const int weight[], int n, int capacity)
{
    if (capacity <= 0 || n <= 0) {
        cout << "kutu kapasitesi ve nesne sayisi pozitif olmalidir.\n";
        return -1;
    ...
    vector<int> sortedWeight(weight, weight + n);
    sort(sortedWeight.rbegin(), sortedWeight.rend()); //nesneleri azalan sirada sirala

    return firstFit(sortedWeight.data(), n, capacity); //siralanmis nesneleri first fit algoritmasina gonder
}
...
int main()
{
    ...
    int result = firstFitDecreasing(weight, n, capacity);

    if (result != -1) {
        cout << "FFD (First Fit Decreasing) icin gerekli kutu sayisi: "<< result << '\n';
    }
    return 0;
}