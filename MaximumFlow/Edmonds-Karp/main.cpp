...
bool bfs(int rGraph[V][V], int s, int t, int parent[])
{
    ...
    //hedef dugume ulasildiysa t, ulasilmadiysa f
    return (visited[t] == true);
}

//edmonds-karp algoritmasini uygular(bfs tabanli ford-fulkerson)
int edmondsKarp(int graph[V][V], int s, int t)
{
    int u, v;

    //residual graph olusturulur
    //baslangicta rGraph orijinal grafin kapasite degerlerinin aynisidir
    int rGraph[V][V];
    ...
    while (bfs(rGraph, s, t, parent))
    {
        ...
        //residual graph'i guncelle (bulunan yoldaki akis gonderildikten sonra)
        for (v = t; v != s; v = parent[v])
        {
            u = parent[v];
            ...
            //geri yondeki kapasiteyi artirilir (daha sonra gonderilen akisin geri alinmasina izin verilir)
